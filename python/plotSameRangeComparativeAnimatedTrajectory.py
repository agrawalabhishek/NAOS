...
inertial_Vx         = data[ 'inertial_velocity_x' ]
inertial_Vy         = data[ 'inertial_velocity_y' ]
inertial_Vz         = data[ 'inertial_velocity_z' ]
...
inertialRange_perturbations = np.abs(np.sqrt( inertial_position_x**2 + inertial_position_y**2 + inertial_position_z**2 ))
inertialRange_noPerturbations = np.abs(np.sqrt( inertial_x**2 + inertial_y**2 + inertial_z**2 ))

rangeToExtract = inertialRange_noPerturbations[ data_indices ].tolist( )
data_indices=[]

for index in range( 0, len( rangeToExtract ) ):
    currentIndex = ( np.abs( inertialRange_perturbations - rangeToExtract[ index ] ) ).argmin( )
    data_indices.append( currentIndex )

rangeExtracted = inertialRange_perturbations[ data_indices ]

nonPerturbingIndices = []
for index in range( 0, len( rangeExtracted ) ):
    currentIndex = ( np.abs( inertialRange_noPerturbations - rangeExtracted[ index ] ) ).argmin( )
    nonPerturbingIndices.append( currentIndex )

# rangeToExtract = inertialRange_noPerturbations[ data_indices ].tolist( )
# data_indices=[]
# for index in range( 0, len( rangeToExtract ) ):
#     currentIndex = np.where( inertialRange_perturbations == rangeToExtract[ index ] )
...
# nonPerturbingIndices = []
# for index in range( 0, len( rangeExtracted ) ):
#     currentIndex = np.where( inertialRange_noPerturbations == rangeExtracted[ index ] )
#     currentIndex = currentIndex[ 0 ]
#     if currentIndex.size != 0:
#         nonPerturbingIndices.append( int( currentIndex ) )
...
# timeValuesExtracted = t_solarPerturbations[ data_indices ].tolist( )

## non-perturbing case
# get the non perturbing data indices now
# nonPerturbingIndices = []
# for index in range( 0, len( timeValuesExtracted ) ):
#     currentIndex = np.where( t == timeValuesExtracted[ index ] )
#     currentIndex = currentIndex[ 0 ]
#     if currentIndex.size != 0:
#         nonPerturbingIndices.append( int( currentIndex ) )

## define the rotation angles for the ellipse
RotationAngles_noSolarPerturbations = ( Wz * t[ nonPerturbingIndices ] ) * 180.0 / np.pi
...
Vz = inertial_Vz[nonPerturbingIndices]
...
y_noSolarPerturbations = inertial_y[ nonPerturbingIndices ].tolist( )
x_noSolarPerturbations = inertial_x[ nonPerturbingIndices ].tolist( )
