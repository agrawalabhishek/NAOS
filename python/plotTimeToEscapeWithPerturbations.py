...
for solarIndex in range( 0, len( uniqueSolarPhases ) ):
    ...
    escape_time                            = data1[ 'time' ]
    escape_azimuth                         = data1[ 'launch_azimuth' ]

    print "Processing data...\n"

    currentUniqueVelocities = np.unique( escape_velocity_magnitude )
    for newIndex in range( 0, len( currentUniqueVelocities ) ):
        currentVelocity = currentUniqueVelocities[ newIndex ]

        newEscapeDataIndices = np.where( escape_velocity_magnitude == currentVelocity )
        newEscapeDataIndices = newEscapeDataIndices[ 0 ]

        plotAzimuths = escape_azimuth[ newEscapeDataIndices ]
        plotTime = escape_time[ newEscapeDataIndices ]
...
#     # separating out data for the current solar phase
#     current_escape_velocity_magnitudes  = escape_velocity_magnitude[ currentEscapeDataIndices ]

#     currentUniqueVelocities = np.unique( current_escape_velocity_magnitudes )
#     for newIndex in range( 0, len( currentUniqueVelocities ) ):
#         currentVelocity = currentUniqueVelocities[ newIndex ]
...
## Show the plot
plt.suptitle( 'Time for regolith to escape, Ellipsoid Longest edge' )
plt.show( )
