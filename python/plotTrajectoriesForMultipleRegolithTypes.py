...
def extractDataFromSQL( databaseConnect ):
    ...
    data = pd.read_sql( "SELECT    inertial_position_x,                                         \
                        ...
                        WHERE      ROUND( initial_velocity_magnitude ) = 13.0                   \
                        AND        ROUND( initial_solar_phase_angle ) = 45.0                    \
                        AND        ROUND( launch_azimuth ) = 330.0;",                           \
                        databaseConnect )
...
ax1.plot( xPositionInertial_data1, yPositionInertial_data1, lw=1.0, label=label1 )
...
ax1.text( xPositionInertial_data4[0], yPositionInertial_data4[0], 'start', fontsize=10, color='black' )

ax1.grid(True)
ax1.set_xlabel('x [m]')
ax1.set_ylabel('y [m]')
ax1.set_title("Trajectories for multiple regolith types for the same initial conditions\n"
                + "$V_{launch}$ = " + str(initial_velocity_data1[ 0 ]) + " [m/s], "
                + "Azimuth = " + str(azimuth_data1[ 0 ]) + " [deg], "
...
ax1.plot( xPositionInertial_data1, yPositionInertial_data1, zPositionInertial_data1,
            zdir = 'z', color='blue', linewidth=1.0 )

ax1.plot( xPositionInertial_data4, yPositionInertial_data4, zPositionInertial_data4,
            zdir = 'z', color='red', linewidth=1.0 )
ax1.text( xPositionInertial_data4[ 0 ], yPositionInertial_data4[ 0 ], zPositionInertial_data4[ 0 ],
            'start', color='black' )
